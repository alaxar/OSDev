...
} __attribute__((packed)) ehci_iTDStatusControl_t;

typedef struct
{
    uintptr_t               nextLP;    // Next Link Pointer: for schedule linkage purposes only
    ehci_iTDStatusControl_t tsc[8];    // control and status for one µ-frame's worth of transactions for a single hispeed-isochronous endpoint
